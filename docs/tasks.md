...
**Подзадачи:**
- [x] Создать метод, вызывающий async через `.Result` в синхронном контексте
- [x] Воспроизвести зависание — убедиться что приложение не отвечает
- [x] Написать комментарий в коде: почему это происходит (SynchronizationContext, захват потока)
- [x] Починить через `await`
- [x] Удалить сломанный код или сохранить в ветке `experiment/deadlock`
